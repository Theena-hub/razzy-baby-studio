...
class Instagram
{
    function Add()
    {
        ...
        if (isset($_FILES['image']) && $_FILES['image']) {
            ...
            if (move_uploaded_file($image_tmp_name, $target_path)) {
                $query = "INSERT INTO tbl_instagram (link,image) VALUES ('".addslashes($link)."','$image_name')";
    ...
    function Fetch()
    {
        ...
        $query = "SELECT * FROM tbl_instagram WHERE id = '$id'";
        $result = mysqli_query($conn, $query);
        if ($result) {
    ...
    function Update()
    {
        ...
        if (isset($_FILES['editimage']) && $_FILES['editimage']['error'] == UPLOAD_ERR_OK) {
            ...
            if (move_uploaded_file($image_tmp_name, $target_path)) {
                ...
                $query = "UPDATE tbl_instagram SET link='".addslashes($editlink)."', image='$image_name' WHERE id='$editId'";                
            }
        }
        else{
            $query = "UPDATE tbl_instagram SET link='".addslashes($editlink)."' WHERE id='$editId'";
        }
    ...
    function Delete()
    {
        ...
        $query = "UPDATE tbl_instagram SET status = 0 WHERE id = '$id'";
        $result = mysqli_query($conn, $query);
        if ($result) {
