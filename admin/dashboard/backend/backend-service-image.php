...
class ServiceImage
{
    function Add()
    {
        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] == UPLOAD_ERR_OK) {
            ...
            if (move_uploaded_file($image_tmp_name, $target_path)) {
                $query = "INSERT INTO tbl_service_images (service_id,image) VALUES ('$serviceId','$image_name')";
    ...
    function Delete()
    {
        ...
        $query = "UPDATE tbl_service_images SET status = 0 WHERE id = '$id'";
        $result = mysqli_query($conn, $query);
        if ($result) {
